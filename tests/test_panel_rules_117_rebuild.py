...
class PanelRulesV22RebuildTests(unittest.TestCase):
    ...
    def test_v22_rebuild_versions_are_synchronized(self) -> None:
        ...
        self.assertEqual(self.contract["panel"]["version"], "1.0.14")
        self.assertEqual(self.contract["app_shell"]["version"], "2.2")
        self.assertEqual(self.contract["panel"]["navigation_contract_version"], "1.3")
        self.assertEqual(self.panel_manifest["panel_version"], "1.0.14")
        self.assertEqual(self.panel_manifest["zoom_policy"]["standard"], "2.2")
        self.assertEqual(self.integration_manifest["version"], "1.0.0-beta068")
        self.assertIn('FRONTEND_UI_VERSION = "1.0.14"', self.runtime)
        self.assertIn('FRONTEND_COMPONENT_SLUG = "v100"', self.runtime)

    def test_header_return_is_parent_only_and_captured_once(self) -> None:
        ...
        self.assertIn("function captureNikasShellReturnRoute({ parentRoute } = {})", self.source_kit)
    ...
    def test_header_contract_declares_home_as_parent(self) -> None:
        header = self.contract["app_shell"]["header"]
        self.assertEqual(header["return_sources"], ["/home/overview"])
        self.assertEqual(header["return_capture_precedence"], ["parent_route"])
        self.assertFalse(header["browser_history_back_allowed"])
